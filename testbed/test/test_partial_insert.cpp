/* 
 * A test program for insert latency with existent keys
 * Insert with existent keys (Figure 10(e))
 */
#include <iostream>
#include <cmath>
...
/******************************* insert data *********************************/
    ...
    cout << CK::MultiThreadAction(&tb, check_num, 1, command::QINSERT, check_start) << endl;
...
/******************************* print results *********************************/
	...
	csv_data[i].not_inserted = 0.1*i;
	csv_data[i].ck_latency = insert_latency;
}

void test_me(int cell_number, int i) {
	int test_thread_num = 1;
...
/******************************* insert data *********************************/
	...
	clock_gettime(CLOCK_MONOTONIC, &time1);
	cout << ME::MultiThreadAction(&tb, check_num, 1, command::QINSERT, check_start) << endl;
	clock_gettime(CLOCK_MONOTONIC, &time2);
...
/******************************* print results *********************************/
    cout << "insert latency: " << insert_latency << endl;
	csv_data[i].me_latency = insert_latency;
}

void test_race(int cell_number, int i) {
	int test_thread_num = 1;
...
/******************************* insert data *********************************/
    ...
    cout << RACE::MultiThreadAction(&tb, check_num, 1, command::QINSERT, check_start) << endl;
...
/******************************* print results *********************************/
	...
	csv_data[i].race_latency = insert_latency;
}

void test_tea(int cell_number, int i) {
	int test_thread_num = 1;
...
/******************************* insert data *********************************/
    ...
    cout << TEA::MultiThreadAction(&tb, check_num, 1, command::QINSERT, check_start) << endl;
...
/******************************* print results *********************************/
	...
	csv_data[i].tea_latency = insert_latency;
}
...
int main(int argc, char **argv) {
	...
	string csv_path = "partial_insert.csv";

	/* Test latency with single client with different non-existent keys
	 * Non-existent keys changes from 0% to 100%
	 */
	for (int i = 0; i <= 10; ++i) {
		cout << "not inserted: " << i*10 << "%" << endl;
		test_ck(cell_number, i);
		send_msg("zero", sock);
		test_me(cell_number, i);
		send_msg("zero", sock);
		test_race(cell_number, i);
		send_msg("zero", sock);
		test_tea(cell_number, i);
		send_msg("zero", sock);
	}
