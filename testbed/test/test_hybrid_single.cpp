/* 
 * A test program for hybrid workloads(Figure 11(a))
 */
#include <iostream>
#include <cmath>
...
double test_ck(int cell_number) {
	...
	int success_num = CK::MultiThreadRun(&tb, &fc, 1);
...
double test_me(int cell_number) {
	...
	int success_num = ME::MultiThreadRun(&tb, &fc, 1);
...
double test_race(int cell_number) {
	...
	int success_num = RACE::MultiThreadRun(&tb, &fc, 1);
...
double test_tea(int cell_number) {
	...
	int success_num = TEA::MultiThreadRun(&tb, &fc, 1);
