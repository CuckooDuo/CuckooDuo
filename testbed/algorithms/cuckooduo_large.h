...
//#define getBucketID(x) ((((indexType)(x)) >> 3) & 0x01fffff)
...
// 1'000'000'000 < 2^30 = 1+26+3
// tbbbbbbbbbbbbbbbbbbbbbbbbbbccc
#define getTableID(x) ((((indexType)(x)) >> 29) & 1)
#define getBucketID(x) ((((indexType)(x)) >> 3) & 0x3ffffff)
#define getCellID(x) (((indexType)(x)) & 7)
#define getIndexType(x, y, z) (((((x) << 26) | (y)) << 3) | (z))
#define emptyIndex (((indexType)0xfffffffe))
#define indexTypeNotFound (((indexType)0xffffffff))
#define maxIndexType ((indexType)0x3fffffff)
...
private:
    ...
    std::vector<std::unordered_map<indexType, indexType> > visit;
    std::vector<std::unordered_map<indexType, int> > visitStep;
    ...
    void bfsSpread(indexType from, int tid){
        ...
        if (!isFixed(table[tableID^1].bucket[altBucketID].fixed, i)){
            ...
            if (visit[vid].find(target) != visit[vid].end()
             || visit[vid][target] != indexTypeNotFound) continue;
