...
#define STASH_SIZE_ME 60000        // size of stash
...
private:
    ...
    inline void ME_RDMA_write(int *bucketID, int sz, const Entry (*entry)[N], int tid){
        for (int i = 0; i < sz; ++i)
            ...
            ra[tid][i] = get_offset_table(0, i, 0, 2);
        //if (entry)
        //    memcpy(ME_write_buf[tid].buf, entry, KV_LEN*N*sz);
        //else
        memset(ME_write_buf[tid].buf, 0, RDMA_KV_LEN*N*sz);
        mod_ME(sz, tid);
        ++viscnt;
    }
    ...
    inline void ME_RDMA_read(int *bucketID, int sz, Entry (*entry)[N], int tid){
        ...
        memset(ME_read_buf[tid].buf, 0, RDMA_KV_LEN*N*sz);
    }
