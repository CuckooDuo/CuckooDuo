...
private:
    ...
    inline void RDMA_write(int tableID, int bucketID, int cellID, const Entry& entry, int tid){
        ...
        ra[tid][0] = get_offset_table(0, tid, cellID, 1);
        ++viscnt;
        ...
        mod_remote(1, tid);
        table[tableID].bucket[bucketID].key[cellID] = *(uint64_t*)entry.key;
        table[tableID].bucket[bucketID].val[cellID] = *(uint64_t*)entry.val;
    }
    ...
    inline void RDMA_read(Entry &entry, int tableID, int bucketID, int cellID, int tid){
        ...
        ++viscnt;
        get_remote(1, tid);
        *(uint64_t*)entry.key = table[tableID].bucket[bucketID].key[cellID];
        *(uint64_t*)entry.val = table[tableID].bucket[bucketID].val[cellID];
    ...
    inline void RDMA_read_bucket(Entry *entry, int tableID, int bucketID, int tid){
        ...
        ra[tid][0] = get_offset_table(0, tid, 0, 1);
        ++viscnt;
        get_bucket(1, 1, tid);
        //memcpy(entry, bucket_buf[tid][0], N*KV_LEN);
        for (int i = 0; i < N; ++i) {
            *(uint64_t*)entry[i].key = table[tableID].bucket[bucketID].key[i];
            *(uint64_t*)entry[i].val = table[tableID].bucket[bucketID].val[i];
        }
    }
    ...
    inline void RDMA_write(int tableID, int bucketID, int cellID, const CK_Entry& entry, int tid){
        ...
        table[tableID].bucket[bucketID].val[cellID] = entry.val;
    }
    // RDMA read, read entry from (table,bucket,cell)
    inline void RDMA_read(CK_Entry &entry, int tableID, int bucketID, int cellID, int tid){
        ...
        entry.key = table[tableID].bucket[bucketID].key[cellID];
        entry.val = table[tableID].bucket[bucketID].val[cellID];
    }
    // RDMA read, read entries from (table,bucket)
    inline void RDMA_read_bucket(CK_Entry *entry, int tableID, int bucketID, int tid){
        ...
        get_bucket(1, 1, tid);
        //memcpy(entry, bucket_buf[tid][0], N*KV_LEN);
        memset(bucket_buf[tid][0], 0, N*RDMA_KV_LEN);
        for (int i = 0; i < N; ++i) {
            entry[i].key = table[tableID].bucket[bucketID].key[i];
            entry[i].val = table[tableID].bucket[bucketID].val[i];
        }
    }
        ...
        if (hash1coll != -1){
            ...
            if (table1coll >= 6){
                ...
                if (table1coll == 6 && fixedCnt == 1){
                    ...
                    ra[tid][0] = get_offset_table(0, 0, entryAID, 1);
                    ra[tid][1] = get_offset_table(0, 0, 6, 1);
                    mod_remote(2, tid);
                    table[TABLE1].bucket[TABLE1].key[entryAID] = allEntry[6].key;
                    table[TABLE1].bucket[TABLE1].key[6] = allEntry[entryAID].key;
                        ...
                        if (i == 6 && fixedCnt == 1){
                            ...
                            if (i == 7 && fixedCnt == 0){
                                ...
                                if (!isFull(current_bucket->full, 6)){
                                    ...
                                    ra[tid][0] = get_offset_table(0, 0, 6, 1);
                                    ra[tid][1] = get_offset_table(0, 0, 7, 1);
                                    mod_remote(2, tid);
                                    table[TABLE1].bucket[TABLE1].key[7] = allEntry[6].key;
                                    table[TABLE1].bucket[TABLE1].key[6] = allEntry[7].key;
                ...
                for (int i=0; i<listCnt; i++){
                    if (i+1<listCnt){
                        ...
                        ra[tid][i] = get_offset_table(
                            ...
                            0, 
                            i, 1);
                        entryList[i].key = table[tmpTableID].bucket[tmpBucketID].key[tmpCellID];
                        ++viscnt;
                    ...
                    for (int i=listCnt-1; i>=0; i--){
                        ...
                        if (tmpTableID == TABLE1){
                            ...
                            if (currentBucketCollision){
                                ...
                                //                                   tmpBucketID, 
                                //                                   tmpCellID, 1);
                                ra[tid][wr_idx] = get_offset_table(0, 
                                                                   0, 
                                                                   i, 1);
                                table[tmpTableID].bucket[tmpBucketID].key[tmpCellID] = entryList[i].key;                              
                                ++wr_idx;
                                ++viscnt;
                            }
                        } else {
...
public:
    ...
    bool deletion(const char* key, int tid, bool test_flag=false) {
        KEY_TYPE InKey = turnInKV(key);
        CK_Entry tmpEntry;
        ...
        cell = check_in_table2(h2, fp);
        if(cell != -1) {
            RDMA_read(tmpEntry, TABLE2, h2, cell, tid);
            if (memcmp(key, &tmpEntry.key, KEY_LEN) != 0) {
            ...
            if (!test_flag) {
                ...
                setFull(table[TABLE2].bucket[h2].full, cell, 1);
            }
        ...
        if(cell != -1) {
            /* RDMA read: table[TABLE1], bucket h1, cell cell to val*/
            RDMA_read(tmpEntry, TABLE1, h1, cell, tid);
            if (memcmp(key, &tmpEntry.key, KEY_LEN) != 0) {
            ...
            if (!test_flag) {
                ...
                setFull(table[TABLE1].bucket[h1].full, cell, 1);
            }
    ...
    bool update(const Entry& OutEntry, int tid) {
        ...
        if(cell != -1) {
            ...
            if (memcmp(OutEntry.key, &tmpEntry.key, KEY_LEN) != 0) {
                bucket_unlock(TABLE2, h2, true);
                return false;
            }
    ...
    /* Insert an entry with check at first */
