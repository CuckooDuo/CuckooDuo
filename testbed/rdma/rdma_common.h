/*
 * Header file for the common RDMA routines used in the server/client program.
 *
 */
...
/* Processes a work completion (WC) notification.
 * @cq_ptr: Completion queue pointer where the notifications are expected to
 * arrive
 * @wc: Array where to hold the work completion elements
 * @max_wc: Maximum number of expected work completion (WC) elements. wc must be
 *          atleast this size.
 */
int my_process_work_completion_events (struct ibv_cq *cq_ptr, 
		struct ibv_wc *wc, int max_wc);
