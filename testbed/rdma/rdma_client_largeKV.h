...
#define RDMA_KEY_LEN 1024
#define RDMA_VAL_LEN 1024
#define RDMA_KV_LEN (RDMA_KEY_LEN + RDMA_VAL_LEN)
struct RDMA_Entry{
    char key[RDMA_KEY_LEN];
    char val[RDMA_VAL_LEN];
};
...
struct entry_BUF_NUM {
	RDMA_Entry buf[BUF_NUM];

	entry_BUF_NUM(const entry_BUF_NUM& b) {
		memcpy(buf, b.buf, BUF_NUM*sizeof(RDMA_Entry));
	}
	entry_BUF_NUM() = default;

	RDMA_Entry& operator [] (int i) {
		return buf[i];
	}
};
/* buffer for two buckets with N entries */
struct entry_2N {
	RDMA_Entry buf[2][2*N];

	entry_2N(const entry_2N& b) {
		memcpy(buf, b.buf, 2*2*N*sizeof(RDMA_Entry));
	}
	entry_2N() = default;
...
struct entry_BUF_ME {
	RDMA_Entry buf[BUF_NUM][N];

	entry_BUF_ME(const entry_BUF_ME& b) {
		memcpy(buf, b.buf, BUF_NUM*N*sizeof(RDMA_Entry));
	}
...
static struct RDMA_Entry clean_buf[2][5][N];
...
static inline RemoteAddr get_offset_table(int table_i, int bucket_i, int cell_i, int type=0) {
  ...
  if (type == 0) {
    ...
    ra.offset = 0;
    ra.offset += (bucket_i%(cell_num/N))*N*RDMA_KV_LEN;
    ra.offset += cell_i*(RDMA_KV_LEN);
  }
  else if (type == 1) {
    // for cuckooduo
  	ra.idx = bucket_i/(cell_num/2/N);

    ra.offset = table_i *(cell_num/2)*(RDMA_KV_LEN);
    ra.offset += (bucket_i%(cell_num/2/N))*N*(RDMA_KV_LEN);
    ra.offset += cell_i*(RDMA_KV_LEN);
  }
  else if (type == 2) {
    // for mapembed
    ...
    ra.offset = 0;
    ra.offset += (bucket_i)*N*RDMA_KV_LEN;
    ra.offset += cell_i*(RDMA_KV_LEN);
  }
