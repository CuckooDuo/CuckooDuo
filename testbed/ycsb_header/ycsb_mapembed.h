...
int inline ExecuteCmd(ME::MapEmbed *tb, int num_ops, command cmd, int tid) {
	int oks = 0;
...
int inline MultiThreadAction(ME::MapEmbed *tb, int total_ops, int num_threads, command cmd) {
	int ret = -1;
...
int inline ExecuteFullCmd(ME::MapEmbed *tb, int num_ops, full_cmd_t *fc, int tid) {
	int oks = 0;
	int offset = num_ops*tid;
	...
	for (int i = 0; i < num_ops; ++i) {
		switch((*(fc->cmd))[offset+i]) {
			case INSERT:
				if (!insert_success)
					break;
				insert_success = tb->insert((*(fc->entry))[offset+i], 0, tid);
				oks += insert_success;
...
int inline MultiThreadRun(ME::MapEmbed *tb, full_cmd_t *fc, int num_threads) {
	int ret = -1;
