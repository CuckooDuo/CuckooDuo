/*
 * Functions to work with YCSB datasets on RACE
 */
#include <future>

#include "../algorithms/race_largeKV.h"
#include "ycsb_read.h"

namespace RACE {
/* Excute commands with entries in type defined by cmd with thread tid */
int inline ExecuteCmd(RACE::RACETable *tb, int num_ops, command cmd, int tid, int start_loc = 0) {
	int oks = 0;
	...
	case DELETE:
		for (int i = 0; i < num_ops; ++i)
			oks += tb->deletion(entry[offset+i].key, tid, true);
		break;
	default:
		break;
	}
...
int inline MultiThreadAction(RACE::RACETable *tb, int total_ops, int num_threads, command cmd, int start_loc = 0) {
	int ret = -1;
...
int inline ExecuteFullCmd(RACE::RACETable *tb, int num_ops, int tid) {
	int oks = 0;
...
int inline MultiThreadRun(RACE::RACETable *tb, int total_ops, int num_threads) {
	int ret = -1;
