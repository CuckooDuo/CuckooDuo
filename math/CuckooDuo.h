...
#define SIG_LEN (MY_FP_LEN/8)           // sig(fingerprint) length: 16 bit
#define TCAM_SIZE 1000000000      // size of TCAM
...
private:
    ...
    int calculate_fp(const char *key, int fphash = 1) {
        if (SIG_LEN == 4) return MurmurHash3_x86_32(key, KEY_LEN, fphash == 1 ? seed_hash_to_fp : seed_hash_to_fp2);
    ...
    rebuildInfo rebuild(const Entry& entry, int balanceFlag = 1){
        ...
        if (noHash1CollisionFlag && (hash1coll == -1) && ((N-(int)table[TABLE1].cell[h[TABLE1]]) < (L-(int)table[TABLE2].cell[h[TABLE2]]))){
            int targetCell = 0;
            while (table[TABLE2].bucket[h[TABLE2]].full[targetCell]) ++targetCell;
            if (targetCell >= N) printf("line309 error\n");
            memcpy(table[TABLE2].bucket[h[TABLE2]].sig[targetCell], sig, SIG_LEN*sizeof(char));
            table[TABLE2].bucket[h[TABLE2]].full[targetCell] = 1;
            RDMA_write(TABLE2, h[TABLE2], targetCell, entry);
        ...
        for (int i=1; i<=fp1gpcnt; i++){
            ...
            for (int j=0; j<cellcnt; j++) if (fp1group[j] == i){
                if (h1cnt >= M && M + h2cnt >= N){
                    // all cell crash in hash1, can not kick
                    return std::make_pair(rebuildError, std::make_pair(emptyEntry, emptyEntry));
                }
                if (fp2group[j] != -1){
                    if (useh1){
                        return std::make_pair(rebuildError, std::make_pair(emptyEntry, emptyEntry));
        ...
        for (int i=0; i<cellcnt; i++) if (pos[i] == -1){
            ...
            if (fp2group[i] == -1 && M + h2cnt < N){
                ...
                if (kickid != -1){
                    if (kickid2 != -1){
                        // unexpected error
                        std::cout << "three element need to kick, error" << std::endl;
                        return std::make_pair(rebuildError, std::make_pair(emptyEntry, emptyEntry));
                    }
                    // two element need to kick, error
                    kickid2 = i;
                } else kickid = i;
            }
        }
        for (int i=0; i<N; i++){
            if (i < h1cnt){
                ...
                RDMA_write(TABLE1, h[TABLE1], i, cellEntry[id[i]]);
        ...
        for (int i=0; i<M; i++) if (table[TABLE1].bucket[h[TABLE1]].full[i]) ++check1;
        for (int i=M; i<N; i++) if (table[TABLE1].bucket[h[TABLE1]].full[i]) ++check2;
        if (check1 != h1cnt) printf("line465 error\n");
        if (check2 != h2cnt) printf("line466 error\n");
        if (h1cnt > M || h2cnt > N-M) printf("line467 error\n");
            ...
            if (!table[tableID].bucket[bucketID].full[cellID]){
                ...
                for (int i=0; i<listCnt; i++){
                    if (i+1<listCnt){
                        ...
                        memcpy(sigList[i], table[indexList[i+1].first.first].bucket[indexList[i+1].first.second].sig[indexList[i+1].second], SIG_LEN*sizeof(char));
                    }
                    else{
                        entryList[i] = entry;
                ...
                if (hash2collisionFlag){
                    ...
                    for (int i=listCnt-1; i>=0; i--){
                        ...
                        if (indexList[i].first.first == TABLE1){
                            ...
                            if (currentBucketCollision){
                                // clear kicked cell
                                if (table[TABLE1].bucket[indexList[i].first.second].full[indexList[i].second]){
                                    table[TABLE1].bucket[indexList[i].first.second].full[indexList[i].second] = 0;
                                ...
                                if (info.first == rebuildOK){
                                    ...
                                    //     printf("unexpected: info.first != rebuildNeedkick\n");
                                    //     cleanVisit();
                                    //     return false;
                                    // }
                                ...
                                memcpy(table[indexList[i].first.first].bucket[indexList[i].first.second].sig[indexList[i].second], sigList[i], SIG_LEN*sizeof(char));
                                RDMA_write(indexList[i].first.first, indexList[i].first.second, indexList[i].second, entryList[i]);
                            }
                        } else {
                            memcpy(table[indexList[i].first.first].bucket[indexList[i].first.second].sig[indexList[i].second], sigList[i], SIG_LEN*sizeof(char));
                            RDMA_write(indexList[i].first.first, indexList[i].first.second, indexList[i].second, entryList[i]);
                    ...
                    if (table[tableID].cell[bucketID] > N) printf("line648 error\n");
                    cleanVisit();
                    return true;
                    // */
                }
                ...
                for (int i=0; i<listCnt; i++){
                    if (i == listCnt - 1){
                        // memcpy(table[indexList[i].first.first].bucket[indexList[i].first.second].sig[indexList[i].second], sig, SIG_LEN*sizeof(char));
                        memcpy(table[indexList[i].first.first].bucket[indexList[i].first.second].sig[indexList[i].second], sigList[i], SIG_LEN*sizeof(char));
                        RDMA_write(indexList[i].first.first, indexList[i].first.second, indexList[i].second, entryList[i]);
                    } else {
                        // memcpy(table[indexList[i].first.first].bucket[indexList[i].first.second].sig[indexList[i].second], 
                        //         table[indexList[i+1].first.first].bucket[indexList[i+1].first.second].sig[indexList[i+1].second], SIG_LEN*sizeof(char));
                        memcpy(table[indexList[i].first.first].bucket[indexList[i].first.second].sig[indexList[i].second], sigList[i], SIG_LEN*sizeof(char));
                        RDMA_write(indexList[i].first.first, indexList[i].first.second, indexList[i].second, entryList[i]);
                    }
                }
                ...
                if (table[tableID].cell[bucketID] > N) printf("line668 error\n");
                cleanVisit();
                return true;
            }
