...
#define TCAM_SIZE 10000000000      // size of TCAM
...
public:
    double calc_empty_bucket_ratio(){
        return (double)emptyNum/(bucket_number*2);
        /*
        int x = 0;
        int y = 0;
        for (int i=0; i<bucket_number; i++){
            y += 2;
            if (table[TABLE1].cell[i] != N) ++x;
            if (table[TABLE2].cell[i] != N) ++x;
        }
        return (double)x/y;
        */
    }
    ...
    bool insert(const Entry& entry) {
        ...
        if(table[TABLE1].cell[h[TABLE1]] < N && table[TABLE1].cell[h[TABLE1]] <= table[TABLE2].cell[h[TABLE2]]) {  //if h1 has more empty cell, insert sig into table 1
            ...
            if(table[TABLE1].cell[h[TABLE1]] == N){
                fullbucket_num++;
                emptyNum--;
            }  
            return true;
            ...
            if(table[TABLE2].cell[h[TABLE2]] == N){
                fullbucket_num++;
                emptyNum--;
            }
            return true;
